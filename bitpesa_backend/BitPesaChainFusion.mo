/**
 * BitPesaChainFusion.mo
 * 
 * This canister handles the cross-chain lending functionality using Chain Fusion.
 * It allows users to:
 * - Deposit Bitcoin as collateral on ICP
 * - Receive USDC loans on Ethereum network
 * - Manage collateral-to-loan ratios across chains
 */
...
import ChainKeyTokenAPI "./ChainKeyTokenAPI";
import Error "mo:base/Error";

// BitPesa Cross-Chain Lending Platform with Chain Fusion
actor class BitPesaChainFusion(init : Types.ChainFusionConfig) {
    ...
    let ckusdcCanister : Principal = init.ckusdc_canister;
    
    // EVM RPC canister 
    let evmRpcCanister : Principal = switch (init.evm_rpc_canister) {
        case (?canister) canister;
        case null Principal.fromText("7hfb6-caaaa-aaaar-qadga-cai"); // Default EVM RPC canister ID
    };
    
    // USDC on Ethereum configuration
    let usdcEthereumConfig : EthereumAPI.Erc20TokenConfig = EthereumAPI.USDC_ETHEREUM;
    ...
    public type AppError = {
        ...
        #InvalidEthereumAddress : Text;
    };
    
    // Loan and user tracking
    ...
    // User Ethereum addresses
    stable var userEthereumAddressesEntries : [(Principal, Text)] = [];
    var userEthereumAddresses = HashMap.HashMap<Principal, Text>(10, Principal.equal, Principal.hash);
    ...
    private func post_upgrade_init() {
        ...
        userEthereumAddresses := HashMap.fromIter(userEthereumAddressesEntries.vals(), userEthereumAddressesEntries.size(), Principal.equal, Principal.hash);
    ...
    system func preupgrade() {
        ...
        userEthereumAddressesEntries := Iter.toArray(userEthereumAddresses.entries());
    ...
    // Set/update user's Ethereum address
    public shared (msg) func setUserEthereumAddress(address : Text) : async Result.Result<(), AppError> {
        // Validate that address starts with 0x and is the right length
        if (not Text.startsWith(address, #text("0x")) or Text.size(address) != 42) {
            return #err(#InvalidEthereumAddress("Invalid Ethereum address format"));
        };
        
        userEthereumAddresses.put(msg.caller, address);
        #ok(());
    };
    
    // Get user's Ethereum address
    public shared query (msg) func getUserEthereumAddress() : async ?Text {
        userEthereumAddresses.get(msg.caller);
    };
    ...
    // =================================================================================================
    // ETHEREUM CHAIN FUSION FUNCTIONS
    // =================================================================================================
    
    // Check USDC balance of an Ethereum address
    public shared func checkUsdcBalanceOnEthereum(address : EthereumAPI.EvmAddress) : async Result.Result<EthereumAPI.TokenAmount, AppError> {
        try {
            let balance = await EthereumAPI.getErc20Balance({
                tokenConfig = usdcEthereumConfig;
                address = address;
                evmRpcCanisterId = ?Principal.toText(evmRpcCanister);
            });
            
            switch (balance) {
                case (#ok(amount)) #ok(amount);
                case (#err(error)) #err(#EthereumError(debug_show(error)));
            };
        } catch (e) {
            #err(#EthereumError(Error.message(e)));
        };
    };
    ...
    public func sendUsdcToEthereum(
        ...
        if (not Text.startsWith(ethereumAddress, #text("0x")) or Text.size(ethereumAddress) != 42) {
            return #err(#InvalidEthereumAddress("Invalid Ethereum address format"));
        };
        
        try {
            // Step 1: Convert ckUSDC to USDC on Ethereum using Chain Fusion
            ...
            switch (conversion) {
                case (#ok(txId)) {
                    // Return the transaction ID or reference
                    #ok(Nat.toText(txId));
                };
                case (#err(error)) {
                    #err(#EthereumError("Failed to convert ckUSDC to Ethereum USDC: " # debug_show(error)));
                };
            };
        } catch (e) {
            #err(#EthereumError("Error sending USDC to Ethereum: " # Error.message(e)));
        };
    };
    
    // Check if a transaction is confirmed on Ethereum
    public func checkEthereumTransactionStatus(txHash : Text) : async Result.Result<Bool, AppError> {
        try {
            let response = await EthereumAPI.getTransactionReceipt(
                txHash, 
                usdcEthereumConfig.chainId,
                ?Principal.toText(evmRpcCanister)
            );
            
            switch (response) {
                case (#ok(receipt)) {
                    // Check if transaction has enough confirmations (usually 12+ for Ethereum)
                    #ok(true);
                };
                case (#err(error)) {
                    #err(#EthereumError("Failed to get transaction receipt: " # debug_show(error)));
                };
            };
        } catch (e) {
            #err(#EthereumError("Error checking transaction status: " # Error.message(e)));
        };
    };
    
    // =================================================================================================
    // CROSS-CHAIN LENDING FUNCTIONS
    // =================================================================================================

    // Create a loan using Bitcoin collateral and receive USDC on Ethereum
    public shared (msg) func createCrossChainLoan(
        loanAmount : Nat,
        durationDays : Nat,
        ethereumAddress : Text
        ...
        // Save user's Ethereum address
        userEthereumAddresses.put(borrower, ethereumAddress);
        ...
        switch (await getBtcUsdPrice()) {
            ...
            case (#ok(btcPrice)) {
                ...
                let requiredCollateralValue = (loanAmount * requiredCollateralRatio) / 100;
                ...
                // Try to send USDC to Ethereum through Chain Fusion
                let txResult = await sendUsdcToEthereum(netLoanAmount, ethereumAddress);
                ...
                switch (transfer_res) {
                    case (#Ok(_)) {
                        ...
                        let updatedLoan = { 
                            ...
                            ethereumAddress = loan.ethereumAddress;
                            ...
                            ethereumTxHash = loan.ethereumTxHash;
                        };
